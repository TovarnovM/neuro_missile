...
class DroneGym3:
    ...
    def make(cls, **kwargs):
        ...
        def pos0_pos_trg_foo():
            ...
            pos_trg = kwargs.get('pos_trg', Vec2.random((-4000, 0),(4000, 2500)))
            return pos0, pos_trg
    ...
    def __init__(self, drone_foo, pos0_pos_trg_foo, **kwargs):
        self.time_curr = 0.0
        self.drone = None 
        ...
        self.vel_max = kwargs.get('vel_max', 45.0)
        self.omega_max = kwargs.get('omega_max', 23.0)
        self.a_max = kwargs.get('a_max', 17.0)
        self.trg_radius = kwargs.get('trg_radius', 10)
        ...
        self.obs_missile_min = np.array(self.obs_min[:])
        self.obs_missile_max = np.array(self.obs_max[:])
        self.history = []
    ...
    def get_observ_abstract(self, drone, trg_drone, state=None, normalize=True):  
        state0 = None
        if state:
        ...
        H = drone.pos.y
        alpha = drone.alpha
        D_vec = trg_drone.pos - drone.pos
        D = D_vec.len()
        V = drone.vel.len()
        
        beta_v = Vec2(1,0).angle_to(drone.vel.rotate(-alpha))
        phi =  Vec2(1,0).angle_to(D_vec.rotate(-alpha))
        omega = drone.omega

        V_trg = trg_drone.vel.len()
        gamma = D_vec.angle_to(trg_drone.vel)
    ...
    def get_observ_missiledrone(self, state=None, normalize=True):
        ...
        """
        return self.get_observ_abstract(self.missiledrone, self.drone, state, normalize)
    ...
    def get_observ(self, state=None, normalize=True):
        ...
        :type state: [type], optional
        """
        return self.get_observ_abstract(self.drone, self.missiledrone, state, normalize)

    def get_delta_t(self):
        try:
            delta_t = self.drone.get_delta_t_minimum(self.missiledrone.pos, self.missiledrone.vel * 1.2, self.vel_max, self.a_max, 1e-5)
            return delta_t
    ...
    def get_delta_t_missiledrone(self):
        try:
            delta_t = self.missiledrone.get_delta_t_minimum_moving_trg(self.drone.pos, self.drone.vel * 1.2, self.drone.vel, self.vel_max, self.a_max, 1e-5)
            return delta_t
        except Exception as e:
    ...
    def is_done_abstract(self, drone, trg_drone):
        drone_pos = drone.pos
        trg_pos = trg_drone.pos
        r = trg_pos - drone_pos
        vel_trg = trg_drone.vel
        drone_vel = drone.vel
        if r.len() < self.trg_radius:
            if r.norm() * drone_vel.norm() > 0.3:
                vel_proj = vel_trg.norm() * drone_vel
                if drone_vel.len() > 1.2 * vel_proj:
                    final_reward = 100
                    return True, final_reward, {
                        'result': 'success', 
                        'final_reward': final_reward
                        }
        if self.antiflip and abs(drone.alpha) > np.pi * 0.75:
        ...
        if abs(drone.omega) > self.omega_max:
            return True, -40, {'result': f'omega too mutch {drone.omega}'}
        return False, 0, {}


    def is_done(self):
        return self.is_done_abstract(self.drone, self.missiledrone)
        

    def is_done_missiledrone(self):
        return self.is_done_abstract(self.missiledrone, self.drone)
    ...
    def step(self, actions, actions_missiledrone):
        ...
        if done:
            if info['result'] == 'success': 
                reward_md -= add_reward
                done_md = True
        elif done_md:
            if info_md['result'] == 'success':
                reward -= add_reward_md
                done = True
    ...
    def plot(self, ax, state=None, drone_mashtb=None, vec_mashtb=None,
            ...
            actions_md=None, ideal_traj_md=False, **kwargs):
        state0 = None
        if state:
            state0 = self.get_state()
            self.set_state(state)

        pos_trg = self.missiledrone.pos
        vel_trg = self.missiledrone.vel
        if drone_mashtb:
        ...
        if ideal_traj:
            ...
            traject = self.drone.get_traject(delta_t, pos_trg, vel_trg)
        ...
        if ideal_traj_md:
            delta_t = self.get_delta_t_missiledrone()
            traject = self.missiledrone.get_traject(
                delta_t, 
                self.drone.pos + delta_t*self.drone.vel, 
                self.drone.vel*1.2)
            ideal_traj_kw = {'ls': ':', 'color': 'orange'}
            ideal_traj_kw = dict(ideal_traj_kw, **kwargs.get('ideal_traj_kw_md', {}))
            ax.plot(traject[:,0], traject[:,1], **ideal_traj_kw)

            if vec_mashtb_md:
                vel = self.drone.vel*1.2 * vec_mashtb
                pos = self.drone.pos + delta_t*self.drone.vel

                vel_kw = {'width': 0.05 * vel.len(), 'color': 'orange', 'alpha':0.3}
                vel_kw = dict(vel_kw, **kwargs.get('vel_kw_md', {}))
                ax.arrow(pos.x, pos.y, vel.x, vel.y, **vel_kw)
