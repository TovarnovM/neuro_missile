...
class DroneGym:
    ...
    def get_delta_t(self):
        try:
            delta_t = self.drone.get_delta_t_minimum(self.pos_trg, self.vel_trg, self.vel_max*100, self.a_max, 1e-4)
    ...
    def get_delta_t_simple(self):
        return (self.drone.pos - self.pos_trg).len()
    ...
    def step(self, actions):
        ...
        :type action: [type]
        """
        delta_t0 = self.get_delta_t_simple()
        F1 = actions[0]
        F2 = actions[1]
        ...
        self.time_curr = self.drone.t
        delta_t1 = self.get_delta_t_simple()

        reward = (delta_t0 - delta_t1) - 1
