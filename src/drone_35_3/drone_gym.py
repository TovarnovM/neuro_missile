...
class DroneGym3:
    ...
    def get_delta_t_missiledrone(self):
        try:
            delta_t = self.missiledrone.get_delta_t_minimum_moving_trg(self.drone.pos, self.drone.vel, self.drone.vel*1.5, self.vel_max*2, self.a_max, 1e-5)
            return delta_t
        except Exception as e:
    ...
    def plot(self, ax, state=None, drone_mashtb=None, vec_mashtb=None,
        ...
        if ideal_traj_md:
            ...
            if vec_mashtb_md:
                vel = self.drone.vel*1.5 * vec_mashtb
