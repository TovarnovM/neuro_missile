...
class DroneGym:
    ...
    def make(cls, **kwargs):
        ...
        def pos0_pos_trg_foo():
            ...
            pos_trg = kwargs.get('pos_trg', Vec2.random((-4000, 0),(4000, 0.01)))
            return pos0, pos_trg

        def vel_trg_foo():
            if vel_trg := kwargs.get('vel_trg'):
                return vel_trg
            vel_trg_len = kwargs.get('vel_trg_len', np.random.uniform(1, 1.5))
            return Vec2(0, -vel_trg_len)
    ...
    def __init__(self, drone_foo, pos0_pos_trg_foo, vel_trg_foo, **kwargs):
        ...
        self.trg_radius = kwargs.get('trg_radius', 5)
    ...
    def is_done(self):
        ...
        if r.len() < self.trg_radius:
            ...
            final_reward = 200 - abs(drone_vel.x) / self.vel_trg.len() * 100
            if  abs(drone_vel.y) > abs(self.vel_trg.y):
                final_reward -= abs(drone_vel.y - self.vel_trg.y)/self.vel_trg.len()*100
